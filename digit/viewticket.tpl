<div class="wdes-block">
    ...
    {if !$invalidTicketId}
        ...
        <div class="wdes-client-ticket panel panel-info panel-collapsable{if !$postingReply} panel-collapsed{/if} hidden-print">
            <div class="panel-head-wdes panel-heading" id="ticketReply">
                <div class="collapse-icon pull-right">
                    <i class="fal fa-{if !$postingreply}plus{else}minus{/if}"></i>
                </div>
                <h3 class="panel-title">
                    <i class="fal fa-pencil-alt"></i> &nbsp; {$LANG.supportticketsreply}
                </h3>
            </div>
            <div class="panel-body{if !$postingReply} panel-body-collapsed{/if}">
                ...
                <form method="post" action="{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;postreply=true" enctype="multipart/form-data" role="form" id="frmReply">
                    ...
                    <div class="row form-group">
                        ...
                        <div class="col-sm-3">
                            <button type="button" class="form-control btn-block" onclick="extraTicketAttachment()">
                                <i class="fal fa-plus"></i> {$LANG.addmore}
                            </button>
                        </div>
                    ...
                    <div class="form-group text-center">
                        <input class="wdes-submit" type="submit" name="save" value="{$LANG.supportticketsticketsubmit}" />
                        <input class="wdes-2nd-btn" type="reset" value="{$LANG.cancel}" onclick="jQuery('#ticketReply').click()" />
                    </div>
        ...
        {foreach $descreplies as $reply}
            <div class="ticket-reply markdown-content{if $reply.admin} staff{/if}">
                ...
                <div class="user">
                    {if $loggedin && not $reply.admin}
                        <img class="gravatar-ticket-img" src="{gravatar email=$client.email rating="PG" }" alt="Avatar" title="Avatar for {$loggedinuser.firstname}" width="40" height="40">
                    {/if}
                    <span class="name">
